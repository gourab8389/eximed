...
export async function queryPineconeVectorStore(
  ...
  try {
    ...
    if (queryResponse.matches.length > 0) {
      ...
      return "No relevant clinical findings found.";
    }
    // return "";
  } catch (err) {
    console.error("Error querying Pinecone vector store:", err);
    return "An error occurred while querying the vector store.";
  }
}
